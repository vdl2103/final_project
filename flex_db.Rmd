...
#In Arlington, Texas, in September, visiting pitchers (left plot, "bottom" of the inning) had slightly slower four-seam fastballs at higher temperatures compared to Rangers' pitchers (left plot, "top" of the inning). Visiting pitchers in Boston in September (right plot, "bottom") had more variable pitch speeds, with slower pitches at lower temperatures, while Red Sox pitchers (right plot, "top") had more consistent pitch speed. 
...
### Comparing Pitch Speed for Home vs. Visiting Pitchers in Texas and Massachusetts 
...
brs9_plotly = ggplotly(brs9)
#Join Graphs

library(patchwork)
subplot(hide_legend(tr9_plotly), hide_legend(brs9_plotly))
subplot(tr9_plotly, brs9_plotly)
```

Column {data-width=350}
-----------------------------------------------------------------------

### Heat Index by US Region
...
### Pitch Speed by Heat Index 

```{r fig.7, warning=FALSE, message=FALSE, eval = FALSE}

# pitch speed (all types) vs. heat index, colored by US region 
#fig_7 = pitch_tidy_db %>% 
  #filter(!pitch_type %in% c("AB", "UN", "NA", "EP", "IN", "FO", "PO", "FS", "KN", "SI", "KC", "SC")) %>% 
